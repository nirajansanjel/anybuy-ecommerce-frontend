...
const ProductCard = ({ product }) => {
  return (
    <div className=" rounded-lg shadow-md bg-white  relative m-1 overflow-hidden dark:bg-slate-500 dark:text-white">
      <Link href={`/products/${product._id}`}>
        <Image
          src={product.imageUrls[0] ?? ImagePlaceHolder}
          className="rounded-t-lg md:h-48 object-cover hover:scale-105 transition-all duration-105"
          alt={product.name}
          height={500}
          width={500}
        />
      ...
      <h3 className="flex text-xl h-10 font-semibold pl-2 my-4 hover:text-primary">
      ...
      <div className="flex justify-between">
        <div className="  ">
          <span className="text-xl pl-2 mb-2">Rs. {product.price}</span>
